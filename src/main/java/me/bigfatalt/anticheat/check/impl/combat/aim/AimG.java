...
@CheckType(label = "Aim G", category = Category.Combat)
@Punishment
    ...
    public void handleCheck(Object object, long timeStamp) {
        if (object instanceof WrappedInFlyingPacket) {
            ...
            if (playerData.movement.deltaYaw > 1.f) {
                if (acceleration > 1.f && acceleration < 10.f && gcd < 131072L && deviation > 2) {
                    vl++;
                ...
                debug("accle+=" + acceleration + " GCD+=" + gcd + " Deviation+=" + deviation);
            }
        }
    }
